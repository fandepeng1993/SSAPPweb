...
//初始化区域图的对象
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(This.$id+"_node_"+This.$max,{name:'',left:X,top:Y,type:This.$nowType,value:'',index:0});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	addNode:function(id,json){
	    ...
	    json.ConnectInCount = 0;
	    json.ConnectOutCount = 0;
	    json.valueType = "node";
	    var mark=json.marked? " item_mark":"";
	    if (json.type.indexOf(" round") < 0) { //任务节点
	    ...
	    if (GooFlow.prototype.color.node) {
	        ...
	        if (json.nodeState == 0) {
	            ...
	            this.$nodeDom[id].css({ "background-color": GooFlow.prototype.nodeColor[json.nodeState], "border-color": GooFlow.prototype.nodeColor[json.nodeState] });
	        }
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
			    This.addLine(This.$id + "_line_" + This.$max, { from: lineStart.id, to: this.id, name: "" },true);
				This.$max++;
			}
			else{
				if(lineStart){
				    This.moveLinePoints(This.$focus, lineStart.id, this.id, "lineStart");
				}else if(lineEnd){
				    This.moveLinePoints(This.$focus, this.id, lineEnd.id, "lineEnd");
				}
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			var t = getElCoordinate(This.$workArea[0]);
			if (This.onTextAreadbClick != null && !This.onTextAreadbClick(id, This)) return;
	...
	loadData:function(data){
		...
		for (var index=1; index <= Object.keys(data.nodes).length; index++) {
		//for(var i in data.nodes) {
			var i = "demo_node_" + index;
			this.addNode(i,data.nodes[i]);
		}
		//for (var index=1; index <= Object.keys(data.lines).length; index++) {
		for(var j in data.lines) {
			//var j = "demo_line_" + index;
			this.addLine(j,data.lines[j],false);
		}
		for (var index=1; index <= Object.keys(data.areas).length; index++) {
		//for(var k in data.areas) {
			var k = "demo_area_" + index;
...
///////////以下为有关画线的方法
	...
	addLineDom:function(id,lineData){
		...
		if(!res)	return;
		//add
		if (this.$nodeData[lineData.from].type == "start round") {
		    this.$nodeData[lineData.to].index = 1;
		    this.sortNode(lineData);
		}
		else if (this.$nodeData[lineData.to].type == "end round") {

		}
		else {
		
		    if (this.$nodeData[lineData.from].index + 1 >= this.$nodeData[lineData.to].index) {
               
		        this.$nodeData[lineData.to].index = this.$nodeData[lineData.from].index + 1;
		        this.sortNode(lineData);
		    }
		}
		if(lineData.type=="sl")
	...
	sortNode: function (lineData)
	{
	    for (var i in this.$lineData) {
	        if (this.$lineData[i].from == lineData.to) {
	            if (this.$nodeData[this.$lineData[i].from].index + 1 > this.$nodeData[this.$lineData[i].to].index) {

	                this.$nodeData[this.$lineData[i].to].index = this.$nodeData[this.$lineData[i].from].index + 1;
	                this.sortNode(this.$lineData[i]);
	            }
	        }
	    }
	},
	//增加一条线
	addLine: function (id, json, ischeck) {
	    if (ischeck) {
	    ...
	    //add：输入输出线数量
		if (this.$nodeData[json.from]) {
		    this.$nodeData[json.from].ConnectOutCount++;
		    this.$lineData[id].index = this.$nodeData[json.from].index+1;
		}
		if (this.$nodeData[json.to]) {
		    this.$nodeData[json.to].ConnectInCount++;
		}
	
		this.$lineData[id].valueType = "line";
		this.$lineData[id].order = json.order;
	...
	delLine:function(id){
	    ...
	    //add
		if (this.$nodeData[this.$lineData[id].from]) {
		    this.$nodeData[this.$lineData[id].from].ConnectOutCount--;
		}
		if (this.$nodeData[this.$lineData[id].to]) {
		    this.$nodeData[this.$lineData[id].to].ConnectInCount--;
		}
	...
	moveLinePoints:function(lineId, newStart, newEnd,lineType, noStack){
		...
		//避免两个节点间不能有一条以上接连线
		for(var k in this.$lineData){
		    if ((newStart == this.$lineData[k].from && newEnd == this.$lineData[k].to) || (newStart == this.$lineData[k].to && newEnd == this.$lineData[k].from))
				return;
		}
	    ...
	    //add
		if (lineType == "lineStart") {
		    if (this.$nodeData[this.$lineData[lineId].to]) {
		        this.$nodeData[this.$lineData[lineId].to].ConnectInCount--;
		    }
		    if (this.$nodeData[newEnd]) {
		        this.$nodeData[newEnd].ConnectInCount++;
		    }
		}
		else if (lineType == "lineEnd") {
		    if (this.$nodeData[this.$lineData[lineId].from]) {
		        this.$nodeData[this.$lineData[lineId].from].ConnectOutCount--;
		    }
		    if (this.$nodeData[newStart]) {
		        this.$nodeData[newStart].ConnectOutCount++;
		        this.$lineData[lineId].index =   this.$nodeData[newStart].index+1;
		    }
		}

		if(this.$undoStack&&!noStack){
		    var paras = [lineId, this.$lineData[lineId].from, this.$lineData[lineId].to, lineType];
	...
	isExist: function (id, type) {
	    if (type == 'node') {
	        if (this.$nodeData[id]) {
	            this.$max++;
	            id = this.$id + "_node_" + this.$max;
	            id= this.isExist(id, type);
	        }
	    }
	    else if (type == 'line') {
	        if (this.$lineData[id]) {
	            this.$max++;
	            id = this.$id + "_line_" + this.$max;
	            id = this.isExist(id, type);
	        }
	    }
