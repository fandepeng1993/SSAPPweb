        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    //自定义部分
                    ///////////////////////
                    if(!alreadyInitProcess) {
                    	//$scope.modelData.processDefinitionKey;
                    	if($scope.modelData.model.properties) {
                    		selectedShape.properties["oryx-process_id"] = $scope.modelData.model.properties["process_id"];
                    		selectedShape.properties["oryx-name"] = $scope.modelData.name;
                    	} else {
                    		selectedShape.properties["oryx-process_id"] = $scope.modelData.processDefinitionKey;
                    		selectedShape.properties["oryx-name"] = $scope.modelData.name;
                    	}
                    	alreadyInitProcess = true;
                    }
                    ///////////////////////
